...
| X-09 | Stage 4's `condition_expression` was free text. The prompt fixed the operator vocabulary but nothing bound the syntax, so C02 held `CONTAINS_ANY(Q1, 'a', 'b')`, `Q5 CONTAINS_ANY ('a','b')` and `CONTAINS_ANY(Q1, ['a','b'])` for one operator, each needing its own regex downstream. Worse, the same prose gave *different operators* on the two identical skeleton documents: R20 was `Q6 != Q5` on C01 and `Q6 NOT IN Q5` on C02. | **Fixed** 30 Aug 2026. The model returns `{question_id, operator, values}` under a closed operator enum; `part2_conditions.render` writes the syntax and is the only place the shape is decided. Both documents now give `Q6 not in Q5`. Covered by `src/test_conditions.py`. |
| X-13 | The same rule was stated twice in two forms. C02 guards Q2 with `Show if: Q1 contains at least one brand` in the questionnaire and formally as R6 in the routing table, and only the routing one was parseable — so a consumer had to prefer routing and fall back to the display column, with nothing keeping the two in agreement. | **Fixed** 30 Aug 2026, on request. Both columns run through one `_resolve_condition`, so the same sentence gives the same string whichever column wrote it. `Question.display_condition_expression` mirrors `RoutingRule.condition_expression`, each keeping its verbatim text and an origin beside it. Verified: 12 agree, 0 mismatch, on both C01 and C02. |
| X-14 | `LLMQuestionFields` had one slot per instruction kind plus a free `other_attributes` dictionary, so a cell stating two instructions of one kind kept only the first, and anything unanticipated arrived under a key the model invented. | **Fixed** 30 Aug 2026, on request. The model returns a classified list under a closed `DirectiveKind`. Validation payloads are still read from their own JSON — a model reading one back is a chance to change a number for nothing. |
| X-15 | Part 2's `_build_guards` re-reads display and routing conditions with its own model call (`propose`), independently of Stage 4, so a prose condition is now read by a model twice through two different mechanisms. | **Open.** Surfaced by X-13; Stage 4 now produces a parseable expression with an origin, which Part 2 could consume where the origin is `derived`. Not changed, because it restructures Part 2 rather than Stage 4. |
| X-12 | Membership was rendered as a call, `contains_any(Q1, ['a','b'])`, putting the question inside the parentheses while every other operator put it on the left. One comparison shape therefore needed a rule of its own downstream, for no reason other than history. | **Fixed** 30 Aug 2026, on request. Every operator is infix: `Q1 contains_any ['a','b']`, `Q5 contains 'x'`, `Q3 answered`. The call form is no longer produced or accepted, and `_call`, `_CALL`, `_CALL_OPS` and `_split_args` are deleted. Verified: one regex reads all 42 conditions across the three fixtures. |
| X-10 | 12 of C02's 20 routing conditions were sent to a model that returned its own input verbatim — the QRE already writes `S1 == 'No'`. The calls bought nothing, spent rate-limit budget, and stamped `origin: inferred` on text the document stated outright, which is the claim CLAUDE.md §14 exists to prevent. All 4 of S01's rules were in this state. | **Fixed** 30 Aug 2026. The deterministic parser is consulted first; where it reads the condition, the expression is rendered from the tree and marked `derived`. C02 went 0 → 15 derived, S01 4 → 0 model calls. |
| X-11 | A squeezed token budget returns an empty completion, which Groq reports as `json_validate_failed` — the same code as genuinely malformed JSON, which must not be retried. C01 crashed the pipeline on it. | **Fixed** 30 Aug 2026. `llm.py` tells them apart by whether `failed_generation` is empty, and retries only the empty case. |
