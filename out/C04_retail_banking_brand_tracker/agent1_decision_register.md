# Decision register — C04_retail_banking_brand_tracker

Source: `C04_retail_banking_brand_tracker.docx` (sha256 `2d882a73f9de982e…`)
...
- **id:** `637cd45fcc43ca72`
- **affects:** R19
- **evidence:** exclusive option selected with another response at Q1 or Q5  ->  ((Q1 contains 'None of these' and Q1 contains_any ['Bank A', 'Bank B', 'Bank C', 'Digital Bank D']) or (Q5 contains 'None of these' and Q5 contains_any ['Branch', 'Mobile app', 'Website', 'Call centre', 'ATM', 'Financial adviser']))
- **current reading:** A prose condition was rewritten into a formal one by a model, and the parser accepted the rewrite.
- **alternatives:** A different formal reading of the same sentence is possible.
- **downstream impact:** Whether this rule or guard fires for a given respondent, so which questions they see and which ending they reach.
- **recommendation:** A person familiar with the survey should confirm the reading recorded here matches what the sentence intends.
...
- **id:** `9e1ec5534198b34b`
- **affects:** Q2
- **evidence:** Q1 contains at least one brand  ->  Q1 contains_any ['Bank A', 'Bank B', 'Bank C', 'Digital Bank D']
- **current reading:** A prose condition was rewritten into a formal one by a model, and the parser accepted the rewrite.
- **alternatives:** A different formal reading of the same sentence is possible.
- **downstream impact:** Whether this rule or guard fires for a given respondent, so which questions they see and which ending they reach.
- **recommendation:** A person familiar with the survey should confirm the reading recorded here matches what the sentence intends.
...
- **id:** `9f8d117be451aa6f`
- **affects:** Q6
- **evidence:** Q5 contains any touchpoint  ->  Q5 contains_any ['Branch', 'Mobile app', 'Website', 'Call centre', 'ATM', 'Financial adviser']
- **current reading:** A prose condition was rewritten into a formal one by a model, and the parser accepted the rewrite.
- **alternatives:** A different formal reading of the same sentence is possible.
- **downstream impact:** Whether this rule or guard fires for a given respondent, so which questions they see and which ending they reach.
- **recommendation:** A person familiar with the survey should confirm the reading recorded here matches what the sentence intends.
